...
public class E_BigSlime_BLUE : ENE_COMMON
{
	...
	private void EneMove()
	{
		...
		typhoon_cnt--;
		if(typhoon_cnt <= 0){
			typhoon_cnt = 340 + UnityEngine.Random.Range(0, 20);
			Vector3 typhoonpos = transform.position;
			typhoonpos.x += UnityEngine.Random.Range(-1.0f, 1.0f);
			Instantiate(pf_typhoon, typhoonpos, Quaternion.identity);
		}
		...
		if (eneshot_cnt <= 0)
		{
			...
			{
				mini_shutugen--;
				if (mini_shutugen < 3) mini_shutugen = 3;
			}
